...
export class ManagerRegisterComponent implements OnInit {
  ...
  insertUser(user: string, pass: string) {
    this.http.post('http://127.0.0.1:3300/addUser/' + user + "/" + pass + "/" + 2, this.httpOptions).subscribe(data => console.log(data));
    console.log("insert user");
  }

  allow(x: string, pwd1: string, pwd2: string): boolean {
    this.http.get('http://127.0.0.1:3300/logins/' + x).subscribe(data => {
